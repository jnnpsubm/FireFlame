...
{
    float3 center = (1.0f / 3.0f)*(gin[0].PosL + gin[1].PosL + gin[2].PosL);
    float3 normal = normalize(gin[0].NormalL + gin[1].NormalL + gin[2].NormalL);

    float3 PosW1 = mul(float4(center, 1.f), gWorld).xyz;
