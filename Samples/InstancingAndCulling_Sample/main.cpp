...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PSTR cmdLine, int showCmd) {
    ...
    InstancingAndCullingApp app(engine);
    try {
        ...
        // application handles
        engine.RegisterUpdateFunc(std::bind(&FLEngineApp4::Update, &app, std::placeholders::_1));
        engine.RegisterWindowResizedHandler(std::bind(&FLEngineApp4::OnGameWindowResized, &app, _1, _2));
        engine.GetWindow()->RegisterMouseHandlers
        (
            std::bind(&FLEngineApp4::OnMouseDown, &app, _1, _2, _3),
            std::bind(&FLEngineApp4::OnMouseUp,   &app, _1, _2, _3),
            std::bind(&FLEngineApp4::OnMouseMove, &app, _1, _2, _3)
        );
