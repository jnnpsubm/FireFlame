...
const std::vector<std::uint8_t> DecryptionKeys::UserDataKeyDs3 =
{
    0xFD, 0x46, 0x4D, 0x69, 0x5E, 0x69, 0xA3, 0x9A,
    0x10, 0xE3, 0x19, 0xA7, 0xAC, 0xE8, 0xB7, 0xFA
};
const std::vector<std::uint8_t> DecryptionKeys::UserDataKeyDs2 =
{
    0xB7, 0xFD, 0x46, 0x3E, 0x4A, 0x9C, 0x11, 0x02,
    0xDF, 0x17, 0x39, 0xE5, 0xF3, 0xB2, 0xA5, 0x0F
};
const std::vector<std::uint8_t> DecryptionKeys::RegulationFileKeyDs3 = 
{
...
const std::vector<std::uint8_t> DecryptionKeys::RegulationFileKeyDs2 =
{
    0x40, 0x17, 0x81, 0x30, 0xDF, 0x0A, 0x94, 0x54,
    0x33, 0x09, 0xE1, 0x71, 0xEC, 0xBF, 0x25, 0x4C
};
const std::vector<std::uint8_t> DecryptionKeys::NetworkSessionKeyDs3 =
{
    0x64, 0x73, 0x33, 0x76, 0x76, 0x68, 0x65, 0x73, 0x30, 0x39, 0x64, 0x6a, 0x78, 0x77, 0x63, 0x6a
};

const std::string DecryptionKeys::Data1Key =
...
const std::string DecryptionKeys::Data2Key =
"-----BEGIN RSA PUBLIC KEY-----\n"
...
const std::string DecryptionKeys::Data3Key =
"-----BEGIN RSA PUBLIC KEY-----\n"
...
const std::string DecryptionKeys::Data4Key =
"-----BEGIN RSA PUBLIC KEY-----\n"
...
const std::string DecryptionKeys::Data5Key =
"-----BEGIN RSA PUBLIC KEY-----\n"
...
const std::string DecryptionKeys::Dlc1Key =
"-----BEGIN RSA PUBLIC KEY-----\n"
...
const std::string DecryptionKeys::Dlc2Key =
"-----BEGIN RSA PUBLIC KEY-----\n"
...
std::unordered_map<std::string, std::string> DecryptionKeys::RsaKeyDictionary =
{
    { "data1.bhd", Data1Key },
    { "data2.bhd", Data2Key },
    { "data3.bhd", Data3Key },
...
std::unordered_map<std::string, std::vector<std::uint8_t>> DecryptionKeys::AesKeyDictionary =
{
    { "regulation.regbnd.dcx.enc", RegulationFileKeyDs3 },
    { "enc_regulation.bnd.dcx", RegulationFileKeyDs2 },
};

bool DecryptionKeys::TryGetRsaFileKey(std::string file, std::string& key)
{
    FireFlame::StringUtils::tolower(file);
    auto& it = RsaKeyDictionary.find(file);
    if (it == RsaKeyDictionary.end()) return false;
    key = it->second;
    return true;
}

bool DecryptionKeys::TryGetAesFileKey(std::string file, std::vector<std::uint8_t>& key)
{
    FireFlame::StringUtils::tolower(file);
    auto& it = AesKeyDictionary.find(file);
