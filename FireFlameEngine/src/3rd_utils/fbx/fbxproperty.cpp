...
FBXProperty::FBXProperty(std::ifstream &input)
{
    ...
    if(type == 'S' || type == 'R') {
        ...
        if(encoding) {
            ...
            std::string compressedBuffer(compressedLength,'\0');
            reader.read(&compressedBuffer[0], compressedLength);

            uint64_t destLen = uncompressedLength;
            uint64_t srcLen = compressedLength;
            uncompress2(decompressedBuffer, (uLongf*)&destLen, (const Bytef*)&compressedBuffer[0], (uLong*)&srcLen);
...
void FBXProperty::write(std::ofstream &output)
{
    ...
    if(type == 'Y') {
        ...
        if(type == 'f') compressedLength = (uint32_t)values.size() * 4;
        else if(type == 'd') compressedLength = (uint32_t)values.size() * 8;
        else if(type == 'l') compressedLength = (uint32_t)values.size() * 8;
        else if(type == 'i') compressedLength = (uint32_t)values.size() * 4;
        else if(type == 'b') compressedLength = (uint32_t)values.size() * 1;
...
uint32_t FBXProperty::getBytes()
{
    ...
    else if(type == 'R') return (uint32_t)raw.size() + 5;
    else if(type == 'S') return (uint32_t)raw.size() + 5;
    else if(type == 'f') return (uint32_t)values.size() * 4 + 13;
    else if(type == 'd') return (uint32_t)values.size() * 8 + 13;
    else if(type == 'l') return (uint32_t)values.size() * 8 + 13;
    else if(type == 'i') return (uint32_t)values.size() * 4 + 13;
    else if(type == 'b') return (uint32_t)values.size() * 1 + 13;
    throw std::string("Invalid property");
}
