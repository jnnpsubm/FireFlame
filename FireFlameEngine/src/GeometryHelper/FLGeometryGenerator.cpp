...
GeometryGenerator::MeshData GeometryGenerator::CreateSphere(float radius, uint32 sliceCount, uint32 stackCount)
{
    ...
    float phiStep = MathHelper::FL_PI / stackCount;
    float thetaStep = MathHelper::FL_2PI / sliceCount;
...
GeometryGenerator::Vertex GeometryGenerator::MidPoint(const Vertex& v0, const Vertex& v1){
    ...
    v.Normal = 0.5f*(v0.Normal + v1.Normal);
    v.Normal.Normalize();
    v.TangentU = 0.5f*(v0.TangentU + v1.TangentU);
    v.TangentU.Normalize();
...
GeometryGenerator::MeshData GeometryGenerator::CreateGeosphere(float radius, uint32 numSubdivisions){
    ...
    for (uint32 i = 0; i < meshData.Vertices.size(); ++i){
        ...
        meshData.Vertices[i].Normal = meshData.Vertices[i].Position.Normalize();
        // Project onto sphere.
        ...
        // Put in [0, 2pi].
        if (theta < 0.0f)
            theta += MathHelper::FL_2PI;
        float phi = acosf(meshData.Vertices[i].Position.y / radius);
        meshData.Vertices[i].TexC.x = theta / MathHelper::FL_2PI;
        meshData.Vertices[i].TexC.y = phi / MathHelper::FL_PI;
...
GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
{
    ...
    for (uint32 i = 0; i < ringCount; ++i)
    {
        ...
        // vertices of ring
        float dTheta = 2.0f*MathHelper::FL_PI / sliceCount;
        for (uint32 j = 0; j <= sliceCount; ++j)
        {
            Vertex vertex;
