...
//***************************************************************************************
#include "FLD3DBilateralBlurFilter.h"
#include "..\FLD3DUtils.h"
#include <assert.h>

namespace FireFlame {
D3DBilateralBlurFilter::D3DBilateralBlurFilter
(
    ID3D12Device* device,
...
ID3D12Resource* D3DBilateralBlurFilter::GetResultResource()
{
    return mBlurTexture0.Get();
}

void D3DBilateralBlurFilter::BuildRootSignature()
{
...
void D3DBilateralBlurFilter::BuildPSOs()
{
    std::string shaderData = 
        "//=============================================================================\n"
        "// Performs a separable bilateral blur with a blur radius up to 5 pixels.\n"
        "//=============================================================================\n"
        "\n"
        ...
        "\n"
        "#define twoSigma2 (2.0f*2.5f*2.5f)\n"
        "#define SCALE 50\n"
        "\n"
        ...
        "    {\n"
        "        int k0 = groupThreadID.x + gBlurRadius;\n"
        "        int k = k0 + i;\n"
        "        //float clr_dis = length(gCache[k0] * SCALE-gCache[k] * SCALE);\n"
        "        float clr_dis = abs(rgb2gray((float3)gCache[k0]) - rgb2gray((float3)gCache[k])) * SCALE;\n"
        "        float s = exp(-clr_dis*clr_dis / twoSigma2);\n"
        "        weights[i + gBlurRadius] *= s;\n"
        "        sum += weights[i + gBlurRadius];\n"
        "    }\n"
        "    for (i = -gBlurRadius; i <= gBlurRadius; ++i)\n"
        "    {\n"
        "        weights[i + gBlurRadius] = weights[i + gBlurRadius] / sum;\n"
        "    }\n"
        "\n"
        ...
        "    for (int i = -gBlurRadius; i <= gBlurRadius; ++i)\n"
        "    {\n"
        "        int k0 = groupThreadID.y + gBlurRadius;\n"
        "        int k = k0 + i;\n"
        "        //float clr_dis = length(gCache[k0] * SCALE - gCache[k] * SCALE);\n"
        "        float clr_dis = abs(rgb2gray((float3)gCache[k0]) - rgb2gray((float3)gCache[k])) * SCALE;\n"
        "        float s = exp(-clr_dis*clr_dis / twoSigma2);\n"
...
void D3DBilateralBlurFilter::BuildDescriptors(UINT descriptorSize)
{
...
void D3DBilateralBlurFilter::OnResize(UINT sampleCount, UINT sampleQuality, UINT newWidth, UINT newHeight)
{
...
std::vector<float> D3DBilateralBlurFilter::CalcGaussWeights()
{
...
void D3DBilateralBlurFilter::BuildDescriptors()
{
...
void D3DBilateralBlurFilter::BuildResources(UINT sampleCount, UINT sampleQuality)
{
