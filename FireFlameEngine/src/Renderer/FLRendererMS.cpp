...
void RendererMS::Initialize(API_Feature api){
	CreateDevice();
	CreateResources();
...
void RendererMS::Tick(){
	m_timer.Tick([&]() {Update(m_timer); });
	Render();
}
// Updates the world.
void RendererMS::Update(StepTimer const& timer){
...
void RendererMS::Clear(){
	...
	D3D12_VIEWPORT viewport = { 0.0f, 0.0f, 
		static_cast<float>(m_renderWnd->ClientWidth()),
		static_cast<float>(m_renderWnd->ClientHeight()), D3D12_MIN_DEPTH, D3D12_MAX_DEPTH };
	D3D12_RECT scissorRect = { 0, 0, m_renderWnd->ClientWidth(), m_renderWnd->ClientHeight() };
	m_commandList->RSSetViewports(1, &viewport);
	m_commandList->RSSetScissorRects(1, &scissorRect);
}

// Submits the command list to the GPU and presents the back buffer contents to the screen.
void RendererMS::Present(){
...
void RendererMS::OnResuming(){
	m_timer.ResetElapsedTime();
	// TODO: Game is being power-resumed (or returning from minimize).
}
void RendererMS::OnWindowSizeChanged(){
	CreateResources();
	// TODO: Game window is being resized.
}
// These are the resources that depend on the device.
void RendererMS::CreateDevice(){
...
void RendererMS::CreateResources(){
	...
	UINT backBufferWidth = static_cast<UINT>(m_renderWnd->ClientWidth());
	UINT backBufferHeight = static_cast<UINT>(m_renderWnd->ClientHeight());
	...
	if (m_swapChain){
		...
		ThrowIfFailed(m_dxgiFactory->MakeWindowAssociation(m_renderWnd->MainWnd(), DXGI_MWA_NO_ALT_ENTER));
	}
...
void RendererMS::WaitForGpu() noexcept{
...
void RendererMS::MoveToNextFrame(){
...
void RendererMS::GetAdapter(IDXGIAdapter1** ppAdapter){
	*ppAdapter = nullptr;
